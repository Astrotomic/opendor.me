...
<div class="flex items-center p-4 space-x-4 bg-white rounded-lg shadow">
    ...
    <div class="space-y-1">
        ...
        <a href="{{ $model->profile_url }}" class="block font-normal leading-tight text-gray-500 hover:text-gray-700 truncate">
            {{ '@'.$model->name }}
        </a>
        @else
        <span class="block font-normal leading-tight text-gray-500 truncate">
            {{ '@'.$model->name }}
        </span>
        @endif
    </div>
