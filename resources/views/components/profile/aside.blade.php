    ...
    <ul class="flex flex-col text-gray-500 md:flex-row md:space-x-4">
        <li>
            <a href="{{ $model->github_url }}" class="flex items-center group">
                ...
                <span class="text-sm font-medium text-gray-900 group-hover:text-indigo-500">{{ $model->name }}</span>
            </a>
        </li>
        @if($model->twitter)
        <li>
            <a href="{{ $model->twitter_url }}" class="flex items-center group">
                ...
                <span class="text-sm font-medium text-gray-900 group-hover:text-indigo-500">{{ '@'.$model->twitter }}</span>
            </a>
        </li>
        @endif
        @if($model->website)
